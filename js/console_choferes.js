...
$("#tabla_choferes").on("click", ".editar", function () {
  ...
  document.getElementById("select_tipo_documento_editar").value =
    data.tipo_documen;
  ...
  document.getElementById("txt_clase_categoria_editar").value =
    data.clase_categoria;
  document.getElementById("txt_nro_licencia_editar").value = data.nro_licencia;
  document.getElementById("txt_fecha_expira_editar").value =
...
function listar_choferes_vencidos() {
  tbl_choferes_dash = $("#tabla_choferes_vencidos").DataTable({
    ...
    columns: [
      ...
      {
        data: null,
        render: (data) => `${data.tipo_documen} - ${data.nro_doc}`,
      },
      { data: "nombres_apellidos" },
      { data: "marca_vehiculo" },
      { data: "nro_licencia" },
    ...
    drawCallback: function () {
      if (tbl_choferes_dash) {
        ...
        console.log(
          `Hoy: ${hoy.toLocaleDateString()}, Es fin de mes: ${esFinDeMes}`
        );
        ...
        tbl_choferes_dash.rows().every(function () {
          ...
          if (diasRestantes < 0) {
            if (esFinDeMes) {
              console.log(
                `Licencia vencida encontrada (${data.nombres_apellidos}): ${diasRestantes} días - Se mostrará porque es fin de mes`
              );
              debeAgregar = true;
            }
          }
          // 🔹 Para alertas PREVENTIVAS: solo mostrar en días EXACTOS (60, 30, 14, 7, 0)
            ...
            if (diasExactos.includes(diasRestantes)) {
              console.log(
                `Alerta preventiva encontrada (${data.nombres_apellidos}): Exactamente ${diasRestantes} días`
              );
              debeAgregar = true;
            }
          }
        ...
        if (alertasActivas.length > 0) {
          console.log(
            "Detalle de alertas:",
            alertasActivas.map(
              (a) => `${a.nombre}: ${a.dias} días (${a.fecha})`
            )
          );
        }
...
function calcularDiasRestantes(fechaVenci) {
  ...
  // Log para debug
  console.log(
    `Fecha vencimiento: ${fechaVenci} -> Convertida: ${venc.toLocaleDateString()} -> Días restantes: ${dias}`
  );
...
function mostrarAlertaVencimiento(data) {
  ...
  if (data.dias < 0) {
    ...
    colorHeader = "#8B0000";
    colorBorde = "#8B0000";
    mensaje = `La licencia del conductor <strong>${
      data.nombre
    }</strong> YA VENCIÓ hace <strong>${Math.abs(data.dias)}</strong> día(s).`;
  } else if (data.dias === 0) {
    // DÍA DEL VENCIMIENTO
