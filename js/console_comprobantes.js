...
// ============================================================
// SOLUCIÓN MEJORADA PARA DROPDOWNS EN DATATABLES
// ============================================================
...
$(document).ready(function() {
  ...
  $(document).on('show.bs.dropdown', '.dropdown', function (e) {
    ...
    let topPos = buttonOffset.top + buttonHeight + 5; // 5px de separación
    let leftPos = buttonOffset.left + buttonWidth - dropdownWidth; // Alineado a la derecha del botón
    
    // Verificar si se sale de la pantalla por abajo
    let windowHeight = $(window).height();
    let scrollTop = $(window).scrollTop();
    
    if (topPos + dropdownHeight > windowHeight + scrollTop) {
      // Mostrar arriba del botón
      topPos = buttonOffset.top - dropdownHeight - 5;
    }
    
    // Verificar si se sale de la pantalla por la izquierda
    if (leftPos < 5) {
      leftPos = buttonOffset.left; // Alineado a la izquierda del botón
    }
    
    // Verificar si se sale de la pantalla por la derecha
    let windowWidth = $(window).width();
    if (leftPos + dropdownWidth > windowWidth - 5) {
      leftPos = windowWidth - dropdownWidth - 5;
    }
    
    // Aplicar posición fija con transición suave
    $dropdown.css({
      'position': 'fixed',
      'top': topPos + 'px',
      'left': leftPos + 'px',
    ...
    $dropdown.addClass('dropdown-positioned');
  });
  
  // ✅ CERRAR DROPDOWN AL HACER SCROLL
  $('.dataTables_scrollBody, .table-responsive, .card-body, window').on('scroll', function() {
    $('.dropdown-menu.show').dropdown('hide');
  });
  
  // ✅ REPOSICIONAR EN CASO DE RESIZE
  ...
  $(document).on('hidden.bs.dropdown', '.dropdown', function () {
    let $dropdown = $(this).find('.dropdown-menu');
    $dropdown.removeClass('dropdown-positioned');
